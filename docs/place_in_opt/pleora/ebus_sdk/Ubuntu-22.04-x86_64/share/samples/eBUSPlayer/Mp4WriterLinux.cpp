...
#if  LIBAVFORMAT_VERSION_MAJOR > 58 || ( LIBAVFORMAT_VERSION_MAJOR == 58 && LIBAVFORMAT_VERSION_MINOR >= 6 )
#else
    av_register_all();
#endif
...
bool Mp4WriterLinux::Open( const std::string &aFilename, PvImage *aImage )
{
...
#else
    mCodecEncode = const_cast<AVCodec *>( avcodec_find_encoder( lCodecId ) );
#endif
    ...
    if ( mCtxEncode != NULL )
    {
        ...
        av_opt_set( mCtxEncode->priv_data, "preset", "ultrafast", 0 ); //ultra fast zero latency is so that there is no need to flush the encoder
        av_opt_set( mCtxEncode->priv_data, "tune", "zerolatency", 0 );
        mCtxEncode->width = mWidth;
        mCtxEncode->height = mHeight;
        mCtxEncode->time_base.num = 1;
        mCtxEncode->time_base.den = mRecordFps;
        mCtxEncode->pix_fmt = AV_PIX_FMT_YUV420P;
    ...
    //If CODEC_FLAG_GLOBAL_HEADER or AV_CODEC_FLAG_GLOBAL_HEADER flag was set in open, the headers (SPS PPS of h264) are written
    ...
    mPictureEncoded = av_frame_alloc();
...
#else
    const AVOutputFormat *lOutputFormat = av_guess_format( 0, aFilename.c_str(), 0 );
#endif
    ...
    strncpy( filename, aFilename.c_str(), 255 );
    mFormatContext->url = filename;
#else
    ...
    if ( pst != NULL && pst->codecpar != NULL )
    {
        AVCodecContext *lContext;
        lContext = avcodec_alloc_context3( mCodecEncode );
        if ( lContext == NULL )
        {
            return false;
        }

        lContext->codec_type = AVMEDIA_TYPE_VIDEO;
        lContext->pix_fmt = AV_PIX_FMT_NV12;
        lContext->codec_id = lCodecId;
        lContext->bit_rate = mAvgBitrate;
        lContext->width = mWidth;
        lContext->height = mHeight;
        // pts clock runs at 90000 hz
        lContext->time_base.num = 1;
        lContext->time_base.den = 90000; 
        lContext->thread_count = 1;

        // setup extradata to be those written by codec into mCtxEncode
        // this should be SPS/PPS of h264
        lContext->extradata_size = mCtxEncode->extradata_size;
        lContext->extradata = mCtxEncode->extradata;

        if( mFormatContext->oformat->flags & AVFMT_GLOBALHEADER )
        {
            lContext->flags |= AV_CODEC_FLAG_GLOBAL_HEADER;
        }

        // copy context parameters into codecpar
        avcodec_parameters_from_context( pst->codecpar, lContext );

        if ( !( mFormatContext->oformat->flags & AVFMT_NOFILE ) )
        {
            if ( avio_open( &mFormatContext->pb, aFilename.c_str(), AVIO_FLAG_WRITE ) )
            {
                return false;
            }
        }
        //this is where the mp4 headers (avcc header is written based on extradata)
        int lResult = avformat_write_header( mFormatContext , NULL );
        if ( lResult < 0 )
        {
            // error
            return false;
...
void Mp4WriterLinux::Close()
{
    ...
    if ( mCtxEncode != NULL )
    {
        ...
        for (int lGotOutput = 1; lGotOutput; )
        {
            ...
            fflush(stdout);
            lGotOutput = 0;
            int lEncResult = avcodec_send_frame( mCtxEncode, NULL );
            while ( lEncResult >= 0 ) 
            {
                lEncResult = avcodec_receive_packet( mCtxEncode, &lAvpkt );
                if ( lEncResult == 0 )
                {
                    lGotOutput = 1;
                    if ( mFormatContext != NULL )
    ...
    if ( mFormatContext != NULL )
    {
        ...
        {
            if (mFormatContext->streams[0]->codecpar)
            {
                //extradata was part of mCtxEncode and will be freed when mCtxEncode is freed
                mFormatContext->streams[0]->codecpar->extradata = NULL;
                mFormatContext->streams[0]->codecpar->extradata_size = 0;
            }
        }
...
bool Mp4WriterLinux::WriteFrame( PvImage *aImage, uint32_t *aFileSizeDelta )
{
    ...
    av_frame_unref( mPictureEncoded );
    mPictureEncoded->format = AV_PIX_FMT_YUV420P;
    ...
    // The pts clock runs at 90000 hz so scale the time stamp
    mPictureEncoded->pts = mVideoTimestamp / ( av_q2d( ( AVRational{1,90000} ) ) * 1000 );
    ...
    int lEncResult = avcodec_send_frame( mCtxEncode, mPictureEncoded );
    if ( lEncResult < 0 ) 
    {
        return false;
    }
    ...
    {
        lEncResult = avcodec_receive_packet( mCtxEncode, &lAvpkt );
        if ( lEncResult == AVERROR(EAGAIN) || lEncResult == AVERROR_EOF )
        {
            continue;
        }
        else if ( lEncResult < 0 )
        {
            return false;
        }
        if ( lAvpkt.size )
