...
#define TAG_BICOLORFILTERING ( "bicolorfiltering" )
...
#define VAL_BICOLORSIMPLE ( "simple" )
#define VAL_BICOLOR3X3 ( "3x3" )
...
ImageFiltering::ImageFiltering()
    ...
    , mBiColorFilter( PvBiColorFilterSimple )
...
PvResult ImageFiltering::Save( PvConfigurationWriter *aWriter )
{
    ...
    // BiColor filtering
    lStr = VAL_BICOLORSIMPLE;
    switch ( mBiColorFilter )
    {
    ...
    aWriter->Store( lStr.c_str(), TAG_BICOLORFILTERING );
...
PvResult ImageFiltering::Load( PvConfigurationReader *aReader )
{
    ...
    // BiColor filtering
    lResult = aReader->Restore( PvString( TAG_BICOLORFILTERING ), lPvStr );
    if ( lResult.IsOK() )
    {
        if ( lPvStr == VAL_BICOLORSIMPLE )
        {
            mBiColorFilter = PvBiColorFilterSimple;
        }
        else if ( lPvStr == VAL_BICOLOR3X3 )
        {
            mBiColorFilter = PvBiColorFilter3X3;
        }
    }
...
void ImageFiltering::ConfigureConverter( PvBufferConverter &aConverter )
{
    ...
    aConverter.SetBiColorFilter( mBiColorFilter );
