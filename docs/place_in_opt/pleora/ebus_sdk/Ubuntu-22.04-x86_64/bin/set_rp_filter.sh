...
DisplayHelp()
{
    ...
    echo "    Allows the eBUS SDK to find GigE Vision devices that"
...
if [ $MODE = "2" ]; then
    echo "Setting rp_filter mode to Loose Reverse Path."
elif [ $MODE = "1" ]; then
    echo "Setting rp_filter mode to Strict Reverse Path."
elif [ $MODE = "0" ]; then
    echo "Setting rp_filter mode to No source validation."
fi 
...
# If net.ipv4.conf.default.rp_filter and net.ipv4.conf.all.rp_filter present in /etc/sysctl.conf, change value to "$MODE"
...
#	and, add "net.ipv4.conf.all.rp_filter = $MODE" to /etc/sysctl.conf
if grep -q ^net.ipv4.conf.all.rp_filter /etc/sysctl.conf; then
    sed -i 's/^net.ipv4.conf.all.rp_filter.*/net.ipv4.conf.all.rp_filter = '$MODE'/g' /etc/sysctl.conf
else
    echo "" >> /etc/sysctl.conf
    echo "net.ipv4.conf.all.rp_filter = $MODE" >> /etc/sysctl.conf
fi
...
if [ "$RESTARTNETWORKSTACK" = "yes" ]; then
    ...
    echo "The network stack will not be restarted. Consider restarting it if you experience any problems."
fi
