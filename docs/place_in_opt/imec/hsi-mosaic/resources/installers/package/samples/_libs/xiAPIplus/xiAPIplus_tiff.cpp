...
static void xiImageGetBitCount(xiAPIplus_Image* image, int* bit_count, int* num_channels, bool* is_rgb)
{
...
void WriteImage(xiAPIplus_Image* image, char* filename)
{
    ...
    line_len = width * ((bits_per_sample / 8) * num_channels) + image->GetXI_IMG()->padding_x;
    ...
    TIFFSetField(tiff_img, TIFFTAG_MAXSAMPLEVALUE, (int)(image->GetXI_IMG()->data_saturation + 0.5));
    ...
    if (image->GetDataFormat() == XI_RAW32FLOAT)
        TIFFSetField(tiff_img, TIFFTAG_SAMPLEFORMAT, SAMPLEFORMAT_IEEEFP);
    else if (image->GetXI_IMG()->data_sign_mode == XI_DATA_SM_SIGNED_2C)
    ...
    else
    {
        ...
        int strip_row = 0;
        int strip_count = 0;
        if (image->GetDataFormat() == XI_RGB24)
        {
            // remove line padding and swap RGB
            for (y = 0; y < height; y++)
            {
                const BYTE* pLine = (const BYTE*)image->GetPixels() + y * line_len;
        ...
        else if (image->GetDataFormat() == XI_RGB32)
        {
        ...
        else if (image->GetDataFormat() == XI_RGB48)
        {
            // remove line padding and swap RGB
            for (y = 0; y < height; y++)
            {
                const USHORT* pLine = (const USHORT*)((BYTE*)image->GetPixels() + y * line_len);
        ...
        else if (image->GetDataFormat() == XI_RGB64)
        {
            // remove line padding, swap RGB, remove A
            for (y = 0; y < height; y++)
            {
                const USHORT* pLine = (const USHORT*)((const BYTE*)image->GetPixels() + y * line_len);
                USHORT* pLineDst = (USHORT*)(pStrip + strip_row * tiff_line_len);
                for (x = 0; x < width; x++)
                {
                    pLineDst[2] = *pLine++;
