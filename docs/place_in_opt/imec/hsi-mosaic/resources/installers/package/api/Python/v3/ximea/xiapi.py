...
class Image(XI_IMG):
    ...
    def get_image_data_numpy(self, invert_rgb_order=False):
        ...
        try:
            ...
            self.frm == XI_IMG_FORMAT["XI_RAW16"].value:
                ...
                c_array = c_uint32*self.width*self.height
                data = c_array()
                c_arr_ops.arr32bit(
                    c_int(self.height),
                    c_int(self.width),
                ...
                numpy_data = np.array(data, copy=False, dtype=np.uint32)
                return numpy_data    
            elif self.frm == XI_IMG_FORMAT["XI_RAW8X2"].value or \
            self.frm == XI_IMG_FORMAT["XI_RAW8X3"].value or \
            ...
            self.frm == XI_IMG_FORMAT["XI_RAW16X3"].value or \
            ...
            self.frm == XI_IMG_FORMAT["XI_RGB_PLANAR"].value:
                ...
                numpy_data = np.array(data, copy=False, dtype=np.uint8)
                return numpy_data
            elif self.frm == XI_IMG_FORMAT["XI_RGB48"].value or \
            self.frm == XI_IMG_FORMAT["XI_RGB16_PLANAR"].value:
                c_array = c_ushort*self.get_channel_count()*self.width*self.height
                data = c_array()
                c_arr_ops.arrRGB_16bit(
                    c_int(self.height),
                    c_int(self.width),
                    c_int(self.padding_x),
                    c_void_p(self.bp),
                ...
                numpy_data = np.array(data, copy=False, dtype=np.uint16)
                return numpy_data
            elif self.frm == XI_IMG_FORMAT["XI_RGB64"].value:
                c_array = c_ushort*self.get_channel_count()*self.width*self.height
                data = c_array()
                c_arr_ops.arrRGBA_16bit(
                    c_int(self.height),
                    c_int(self.width),
                    c_int(self.padding_x),
                    c_void_p(self.bp),
    ...
    def get_bytes_per_pixel(self):
        ...
        if self.frm == XI_IMG_FORMAT["XI_MONO8"].value:
            ...
            return 2
        elif self.frm == XI_IMG_FORMAT["XI_RAW8X3"].value:
            return 3            
            ...
            return 4
        elif self.frm == XI_IMG_FORMAT["XI_RAW16X3"].value:
            return 6            
            ...
            return 4
        elif self.frm == XI_IMG_FORMAT["XI_RGB48"].value:
            return 6
        elif self.frm == XI_IMG_FORMAT["XI_RGB64"].value:
            return 8
        elif self.frm == XI_IMG_FORMAT["XI_RGB_PLANAR"].value:
            return 3
        elif self.frm == XI_IMG_FORMAT["XI_RGB16_PLANAR"].value:
            return 6
        else:
            raise Xi_error(108)     #"Data format not supported"
    ...
    def get_channel_count(self):
        ...
        self.frm == XI_IMG_FORMAT["XI_RAW16X2"].value:
            return 2
        elif self.frm == XI_IMG_FORMAT["XI_RAW8X3"].value or \
        self.frm == XI_IMG_FORMAT["XI_RAW16X3"].value or \
        self.frm == XI_IMG_FORMAT["XI_RGB24"].value or \
        self.frm == XI_IMG_FORMAT["XI_RGB48"].value or \
        self.frm == XI_IMG_FORMAT["XI_RGB_PLANAR"].value  or \
        self.frm == XI_IMG_FORMAT["XI_RGB16_PLANAR"].value:
            return 3
        elif self.frm == XI_IMG_FORMAT["XI_RAW8X4"].value or \
        self.frm == XI_IMG_FORMAT["XI_RAW16X4"].value or \
        self.frm == XI_IMG_FORMAT["XI_RGB32"].value or \
        self.frm == XI_IMG_FORMAT["XI_RGB64"].value:
            return 4
        else:
            raise Xi_error(108)     #"Data format not supported"
...
#-------------------------------------------------------------------------------------------------------------------
    ...
    def get_aeag_skip_frames_count(self):
        '''
        Minimum number of frames to be skipped between AEAG compensation and AEAG re-evaluationXI_PRM_AEAG_SKIP_FRAMES_COUNT
        '''
        return self.get_param('aeag_skip_frames_count')

    def get_aeag_skip_frames_count_maximum(self):
        '''
        Minimum number of frames to be skipped between AEAG compensation and AEAG re-evaluationXI_PRM_AEAG_SKIP_FRAMES_COUNT
        '''
        return self.get_param('aeag_skip_frames_count:max')

    def get_aeag_skip_frames_count_minimum(self):
        '''
        Minimum number of frames to be skipped between AEAG compensation and AEAG re-evaluationXI_PRM_AEAG_SKIP_FRAMES_COUNT
        '''
        return self.get_param('aeag_skip_frames_count:min')

    def get_aeag_skip_frames_count_increment(self):
        '''
        Minimum number of frames to be skipped between AEAG compensation and AEAG re-evaluationXI_PRM_AEAG_SKIP_FRAMES_COUNT
        '''
        return self.get_param('aeag_skip_frames_count:inc')

    def set_aeag_skip_frames_count(self, aeag_skip_frames_count):
        '''
        Minimum number of frames to be skipped between AEAG compensation and AEAG re-evaluationXI_PRM_AEAG_SKIP_FRAMES_COUNT
        '''
        self.set_param('aeag_skip_frames_count', aeag_skip_frames_count)
...
#-------------------------------------------------------------------------------------------------------------------
    ...
    def get_gpi_level_at_image_readout_end(self):
        '''
        GPI Level at image readout endXI_PRM_GPI_LEVEL_AT_IMAGE_READOUT_END
        '''
        return self.get_param('gpi_level_at_image_readout_end')

    def get_gpi_level_at_image_readout_end_maximum(self):
        '''
        GPI Level at image readout endXI_PRM_GPI_LEVEL_AT_IMAGE_READOUT_END
        '''
        return self.get_param('gpi_level_at_image_readout_end:max')

    def get_gpi_level_at_image_readout_end_minimum(self):
        '''
        GPI Level at image readout endXI_PRM_GPI_LEVEL_AT_IMAGE_READOUT_END
        '''
        return self.get_param('gpi_level_at_image_readout_end:min')

    def get_gpi_level_at_image_readout_end_increment(self):
        '''
        GPI Level at image readout endXI_PRM_GPI_LEVEL_AT_IMAGE_READOUT_END
        '''
        return self.get_param('gpi_level_at_image_readout_end:inc')
    ...
    def get_metadata_sampling_mode(self):
        '''
        Defines metadata sampling modeXI_PRM_METADATA_SAMPLING_MODE
        '''
        return self.get_param('metadata_sampling_mode')

    def get_metadata_sampling_mode_maximum(self):
        '''
        Defines metadata sampling modeXI_PRM_METADATA_SAMPLING_MODE
        '''
        return self.get_param('metadata_sampling_mode:max')

    def get_metadata_sampling_mode_minimum(self):
        '''
        Defines metadata sampling modeXI_PRM_METADATA_SAMPLING_MODE
        '''
        return self.get_param('metadata_sampling_mode:min')

    def get_metadata_sampling_mode_increment(self):
        '''
        Defines metadata sampling modeXI_PRM_METADATA_SAMPLING_MODE
        '''
        return self.get_param('metadata_sampling_mode:inc')

    def set_metadata_sampling_mode(self, metadata_sampling_mode):
        '''
        Defines metadata sampling modeXI_PRM_METADATA_SAMPLING_MODE
        '''
        self.set_param('metadata_sampling_mode', metadata_sampling_mode)
