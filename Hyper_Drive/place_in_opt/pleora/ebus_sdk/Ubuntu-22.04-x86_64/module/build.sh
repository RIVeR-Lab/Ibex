...
# for ebUniversalProForEthernet-x86_64.ko 7.5.3 build 7155
#
################################################################################
...
DisplayHelp()
{
    ...
    $ECHO_BIN ""
    $ECHO_BIN "COPYRIGHT"
    $ECHO_BIN "    Pleora Technologies Inc. Copyright (c) 2002-2025"
    $ECHO_BIN ""
    $ECHO_BIN "VERSION"
    $ECHO_BIN "    7.5.3 build 7155"
    $ECHO_BIN ""
}
...
if [ "$KVER" = "4.4.0" -a "$PATCH" -ge 143 ] ; then
   export EXTRA_PLEORA_CFLAGS=-DUBUNTU_PATCH
fi

if [ "$KVER" = "4.18.0" -a $(echo $PATCH | cut -d'.' -f 1) -ge 193 ] ; then
   export EXTRA_PLEORA_CFLAGS=-DCENTOS_PATCH
fi
...
$ECHO_BIN " " > "$SRC_FOLDER"/.libebTransportLayer-x86_64.o.cmd  # File is required to build module
if [ -z "$TOOLS" ]; then
